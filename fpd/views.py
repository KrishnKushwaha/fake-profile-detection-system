...
insta_password = 'YOUR_PASSWORD_HERE'
...
def load_session_from_browser(L):
    ...
    try:
        # ⚠️ CRITICAL: YOU MUST REPLACE THIS sessionid VALUE WITH YOUR FRESH COOKIE ⚠️
        cookies = {
            "sessionid": "REPLACE_ME_WITH_YOUR_NEW_SESSIONID_FROM_BROWSER",
        }
...
def login_to_instagram():
    ...
    # Load session safely to bypass 400 Bad Request
    load_session_from_browser(L)
    return L
...
def insta(request):
    """Renders the Instagram data page."""
    return render(request, 'fpd/instagram.html')
